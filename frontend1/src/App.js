...
const MeasurementResults = ({ measurementData, calculatedPoints, navigateBack, navigateToCalculatedPoints }) => {
  ...
  const handleCalculateAngle = () => {
    ...
    if (selectedPoints.length === 3) {
      ...
      // Calculate arc radius (can be adjusted)
      ...
      // Calculate arc properties for visualization
      const distance = Math.sqrt(
        Math.pow(p2.x - p1.x, 2) + Math.pow(p2.y - p1.y, 2)
      );
      const arcRadius = distance * 0.2;
      
      // Create reference vector (1, 0) for x-axis
      const v1 = { x: 1, y: 0 };
      ...
      const endPoint = {
        x: p1.x + nv2.x * arcRadius,
        y: p1.y + nv2.y * arcRadius
      };
          ...
          <div className="coordinate-plot-container">
            ...
            >
              ...
              {lines.map((line, index) => (
                ...
                {/* Draw angle arc with enhanced visualization */}
                {angleArc && (
                  <ReferenceLine
                    key="angle-arc"
                    ifOverflow="visible"
                      ...
                      try {
                        ...
                        return (
                          <g>
                            ...
                            <path
                              d={`M ${startX},${startY} 
                                  A ${radius},${radius} 0 ${arcLargeFlag} ${sweepFlag} ${endX},${endY}`}
                              fill="none"
                              stroke={color}
                              strokeWidth={2}
                              className="angle-arc"
