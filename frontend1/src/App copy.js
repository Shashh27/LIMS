...
const MeasurementResults = ({ measurementData, calculatedPoints, navigateBack, navigateToCalculatedPoints }) => {
  ...
  // Distance calculation between two points (unchanged)
  ...
  // Angle calculation between three points (unchanged)
 ...
 // Modified handleCalculateAngle function with enhanced arc visualization
const handleCalculateAngle = () => {
  ...
  if (selectedPoints.length === 3) {
    ...
    const p2Index = selectedPoints[1];
    ...
    const endAngle = Math.atan2(v3y, v3x);
    ...
    // Create result text
    let resultsText = '';
    resultsText += `${p1Index + 1}: ${p1.x.toFixed(2)}, ${p1.y.toFixed(2)}\n`;
    resultsText += `${p2Index + 1}: ${p2.x.toFixed(2)}, ${p2.y.toFixed(2)}\n`;
    resultsText += `${p3Index + 1}: ${p3.x.toFixed(2)}, ${p3.y.toFixed(2)}\n\n`;
    resultsText += `Angle at ${p2Index + 1}: ${angle.toFixed(2)}°`;
    
    setResult(resultsText);
    
    // Set the angle arc with angle value included
    setAngleArc({
      cx: p2.x,
      cy: p2.y,
      radius: p2.radius,
    ...
    resultsText += `Angle with x-axis: ${Math.abs(angle).toFixed(2)}°`;
    
    setResult(resultsText);
    
    // Add an arc visualization for the angle with x-axis
    const arcRadius = calculateDistance(p1, p2) * 0.3; // 30% of distance
    const startAngle = 0; // X-axis
...
const generateAngleArcPath = (angleArc) => {
  ...
  const { cx, cy, radius, startAngle, endAngle, sweepFlag } = angleArc;
          ...
          <div className="coordinate-plot-container">
            ...
            >
              ...
              {lines.map((line, index) => (
                ...
                {/* Draw angle arc with angle text */}
                {angleArc && (
                  <ReferenceLine
                    key="angle-arc"
                    ifOverflow="visible"
                    segment={[
                      ...
                      { x: angleArc.cx + Math.cos(angleArc.startAngle) * 0.1, 
                        y: angleArc.cy + Math.sin(angleArc.startAngle) * 0.1 }
                    ]}
                    shape={(props) => {
                      if (!props.xAxis?.scale || !props.yAxis?.scale) return null;
                      
                      const { xAxis, yAxis } = props;
                      const { cx, cy, radius, startAngle, endAngle, sweepFlag, angle } = angleArc;
                      
                      // Calculate the arc radius (make it smaller than the circle radius)
                      ...
                      try {
                        ...
                        // Calculate points in data coordinates
                        ...
                        // Convert to pixel coordinates
                        const startXPx = xAxis.scale(startX);
                        const startYPx = yAxis.scale(startY);
                        const endXPx = xAxis.scale(endX);
                        const endYPx = yAxis.scale(endY);
                        ...
                        const radiusX = Math.abs(xAxis.scale(cx + arcRadius) - xAxis.scale(cx));
                        const radiusY = Math.abs(yAxis.scale(cy) - yAxis.scale(cy + arcRadius));
                        
                        // Calculate text position
                        const midAngle = (startAngle + endAngle) / 2;
                        const textRadius = arcRadius * 1.2;
                        ...
                        return (
                          <g>
                            <path
                              d={`M ${x},${y} 
                                 L ${startXPx},${startYPx} 
                                 A ${radiusX},${radiusY} 0 ${largeArcFlag} ${sweepFlag} ${endXPx},${endYPx} 
                                 L ${x},${y}`}
