  ...
  const handleSubmit = async () => {
    try {
      ...
      const formData = {
        ...
        values: data
          .filter(item => item.actual_size && item.first_set && item.second_set && item.third_set)
          .map(item => ({
            actual_size_of_setting_plug_gauge: item.actual_size,
            first_set_error: item.first_set,
            second_set_error: item.second_set,
            third_set_error: item.third_set,
            average_error: calculateAverage(item)
          }))
      };
